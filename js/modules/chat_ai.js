...
function buildActivityProactiveTask(chat, task) {
    if (task?.phase === 'call_followup') {
        return [
            '<call_followup>',
            `你刚刚连续拨打了 ${task.attempts} 次${task.callType === 'video' ? '视频' : '语音'}电话。每次都是等待30秒无人接听后，由你结束本次拨号；对方没有接听，也没有主动挂断或明确拒绝。`,
            '这是一件刚发生的通话事件，不是要求你重新回复历史里最后一条用户消息。',
            '如果现在要发普通消息，应围绕对方一直没接电话这一事实自然反应，不要误解成对方挂了你的电话。',
            '不要质问为什么挂电话或为什么挂我电话；可以根据人物性格、关系和语境考虑为什么不接、没看到吗、在干嘛等未接来电语义。',
