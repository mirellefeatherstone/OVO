...
2. [吐舌头] / [鬼脸] 从 WebWeChat 官方 Unicode Emoji 雪碧图裁切。
3. 其余雪碧图没有的新版表情，从 wechat-emoji-parser 的独立 PNG 资源补齐。
4. 保留 assets/wechat-emoji/order.json 不动，只覆盖同名 PNG。
...
# =========================================================
# 2. WebWeChat 官方 Unicode Emoji 雪碧图
# =========================================================

# WebWeChat 对“吐舌 / 鬼魂”等标准 Unicode emoji 使用另一张腾讯雪碧图。
OFFICIAL_UNICODE_SPRITE_URL = (
    "https://res.wx.qq.com/a/wx_fed/webwx/res/static/img/1BJVkIX.png"
)

# CSS 官方映射：
#   U+1F445 吐舌  -> background-position: 0 -3000px
#   U+1F47B 鬼魂  -> background-position: 0 -1880px
#
# 我们的 token 名称沿用当前 order.json：
#   [吐舌头] -> 官方“吐舌”
#   [鬼脸]   -> 官方“鬼魂”
UNICODE_EMOJI_OFFSETS = {
    "吐舌头": 3000,
    "鬼脸": 1880,
}

UNICODE_LOGICAL_CELL = 20
...
def load_official_unicode_sprite():
    cache_path = SOURCE_CACHE_DIR / "webwechat_unicode_emoji.png"
    ...
    try:
        if cache_path.exists():
            ...
            print(
                "正在下载腾讯 WebWeChat Unicode Emoji 雪碧图：\n"
                f"  {OFFICIAL_UNICODE_SPRITE_URL}"
            )
            raw = download_bytes(OFFICIAL_UNICODE_SPRITE_URL)
            cache_path.write_bytes(raw)
            print(f"✓ 已缓存：{cache_path}")
        ...
        print(
            "× 官方 Unicode Emoji 雪碧图下载/读取失败："
            f"{type(exc).__name__}: {exc}"
        )
        return None


def crop_unicode_emoji(sprite: Image.Image, y_offset: int) -> Image.Image:
    """
    官方 CSS 逻辑尺寸为 20×20，整张图纵向排列。
    根据实际图片宽度自动兼容 1x / 2x。
    """
    scale = sprite.width / UNICODE_LOGICAL_CELL

    if scale <= 0:
        raise ValueError("Unicode 雪碧图缩放比例异常")

    left = 0
    top = round(y_offset * scale)
    right = round(UNICODE_LOGICAL_CELL * scale)
    bottom = round((y_offset + UNICODE_LOGICAL_CELL) * scale)

    if right > sprite.width or bottom > sprite.height:
        raise ValueError(
            f"Unicode emoji 超出雪碧图范围："
            f"{left},{top},{right},{bottom} / {sprite.size}"
        )
...
def main():
    ...
    for n, raw_name in enumerate(order, 1):
        ...
        # B. 腾讯 WebWeChat 官方 Unicode Emoji
        if image is None and name in UNICODE_EMOJI_OFFSETS:
            if unicode_sprite is not None:
                try:
                    y_offset = UNICODE_EMOJI_OFFSETS[name]
                    image = crop_unicode_emoji(
                        unicode_sprite,
                        y_offset,
                    )
                    source_label = "腾讯 WebWeChat Unicode Emoji"
                    unicode_official_count += 1
                except Exception as exc:
                    print(
                        f"[{n:02d}/{len(order)}] {name}.png "
                        f"Unicode 官方裁切失败：{exc}"
                    )
    ...
    print("\n" + "=" * 62)
    print("替换完成")
    print(f"腾讯 WebWeChat QQFace：     {official_count} 张")
    print(f"腾讯 WebWeChat Unicode：    {unicode_official_count} 张")
